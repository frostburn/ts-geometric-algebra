...
describe('Geometric Algebra', () => {
    ...
    for (let p = 0; p < 3; ++p) {
      for (let q = 0; q < 3; ++q) {
        for (let r = 0; r < 2; ++r) {
          ...
          expect(a.add(s).exp().closeTo(a.exp().mul(s.exp()), 0.1)).toBeTruthy();

          const d = a.scale(0.25);
          const e = b.scale(0.25);
          expect(d.add(e).exp().closeTo(d.scale(1/256).exp().mul(e.scale(1/256).exp()).pow(256), 0.1)).toBeTruthy();
        }
      }
    }
  });
  ...
  it('can be raised to an integer power', () => {
    ...
    for (let i = 0; i < 10; ++i) {
      ...
      expect(a.pow(3).closeTo(power, 0.01)).toBeTruthy(); power = power.mul(a);
      expect(a.pow(4).closeTo(power, 0.05)).toBeTruthy(); power = power.mul(a);
      expect(a.pow(5).closeTo(power, 0.1)).toBeTruthy(); power = power.mul(a);
      expect(a.pow(6).closeTo(power, 0.1)).toBeTruthy(); power = power.mul(a);
    }
  });
});
